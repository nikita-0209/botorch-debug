...
class NN_Model(Model):
    ...
    def posterior(self, X, observation_noise = False, posterior_transform = None):
        ...
        # var = var.unsqueeze(-1)
        """
        Remark: issue arises because posterior is called twice.
        first for the sampler:
            now when sampler calls, then mean = 10, var = 10
        but when forward class
            then mean = 10*1 and var= 10x1
        """
        if len(X.shape)==2:
